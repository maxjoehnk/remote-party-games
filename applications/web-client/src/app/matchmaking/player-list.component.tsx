...
const TeamList = ({ teams, players, canChangeTeam }) => {
    ...
    return <div className="player-list__team-list">
        {teams.map(t => <Team key={t.id} team={t} players={players} canChangeTeam={canChangeTeam && !t.players.includes(user.id)}/>)}
    </div>;
};
...
const Team = ({ team, players, canChangeTeam }) => {
    ...
    return <div>
        <h3 className="player-list__team-header">
            ...
            {canChangeTeam && <button className="player-list__join-team-btn" onClick={() => switchTeam(team.id)}>{i18n`Join Team`}</button>}
        </h3>
