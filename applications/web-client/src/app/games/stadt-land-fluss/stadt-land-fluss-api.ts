...
export function subscribeStadtLandFlussGameUpdates(callback: (state: StadtLandFlussGameState) => void) {
  const startSubscription = onMessage('stadt-land-fluss/round-started', msg => callback(msg.gameState));
  const finishSubscription = onMessage('stadt-land-fluss/round-finished', msg => callback(msg.gameState));
  const scoreSubscription = onMessage('stadt-land-fluss/score-updated', msg => callback(msg.gameState));
